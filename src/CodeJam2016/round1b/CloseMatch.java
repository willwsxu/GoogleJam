...
public class CloseMatch {
    ...
    void greedy(String C, String J)
    {
        ...
        for (int i=0; i<len; i++) {
            int lookahead=0;
            if (i<len-1) {
                if (C.charAt(i+c+1)!='?' && J.charAt(i+j+1)!='?') {
                    lookahead = C.charAt(i+c+1)-J.charAt(i+j+1);
                }
            }
            if (C.charAt(i+c)=='?' && J.charAt(i+j)=='?') {
                if (cmp<0) {
                    newC.append('9');
                    newJ.append('0');
                } else if (cmp>0) {
                ...
                newJ.append(J.charAt(i+j));     
            }
            else if (J.charAt(i+j)=='?') {
                if (cmp<0) {
                    newJ.append('0');
                } else if (cmp>0) {
                    newJ.append('9');                    
                } else {
                    char ch=C.charAt(i+c);
                ...
                newC.append(C.charAt(i+c));
            }
            else {
                if (cmp==0 ) {
                    cmp = C.charAt(i+c)>J.charAt(i+j)? 1:-1;
                }
                newC.append(C.charAt(i+c));
                newJ.append(J.charAt(i+j));  
            }
